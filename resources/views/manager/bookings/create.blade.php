...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-600 mt-1">Create a booking for a client</p>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form method="POST" action="{{ route('manager.bookings.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select name="property_id" id="property_id" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm">
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" data-price="{{ $property->price_per_night }}" {{ old('property_id') == $property->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date') }}" required min="{{ date('Y-m-d') }}" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm">
                        @error('start_date')
                            <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm">
                        @error('end_date')
                            <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="total_price" id="total_price" value="{{ old('total_price') }}" step="0.01" min="0" class="w-full pl-8 pr-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm" placeholder="0.00">
                    </div>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm" placeholder="Add any additional notes about this booking...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
